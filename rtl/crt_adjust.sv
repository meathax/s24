...
(
    ...
    // Keep the intentional modulo-2^AW address wrap explicit.  This avoids
    // relying on an implicit narrowing conversion in Quartus.
    wire signed [AW+1:0] rd_addr_full = rdcnt_s - hoff_s;
    wire [AW-1:0] rd_addr = rd_addr_full[AW-1:0];
    always @(posedge clk) if (pxl2_cen) begin
