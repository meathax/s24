...
//  the active display: one selector digit, a colon, then six value digits
//  (a zero-padded 24-bit value). Purely combinational on hcount/vcount, so
//  it costs no extra pipeline latency and needs no framebuffer of its own --
...
module s24_debug_overlay (
    ...
    localparam int N_CHARS = 8;   // sel digit, ':', 6 value digits
    ...
    always_comb begin
        ...
        char_index = rel_x[5:3];
        col_in_char = rel_x[2:0];
        row_in_char = rel_y[2:0];
        case(char_index)
            3'd0: glyph = {1'b0,sel};                   // selector digit
            3'd1: glyph = 5'h10;                         // ':'
            3'd2: glyph = {1'b0,value[23:20]};
            3'd3: glyph = {1'b0,value[19:16]};
            3'd4: glyph = {1'b0,value[15:12]};
            3'd5: glyph = {1'b0,value[11:8]};
            3'd6: glyph = {1'b0,value[7:4]};
            default: glyph = {1'b0,value[3:0]};
        endcase
