...
`endif	
	...
	// Instruction-boundary observation for integrated protection devices.
	// instr_start marks IR -> IRD: the prefetched word is now actually being
	// accepted for execution, rather than merely appearing on a program read.
	output instr_start,
	output [15:0] instr_opcode,
	output [23:1] instr_address
	);
	...
	reg [15:0] Irc, Ir, Ird;
	reg addrXToIrc;
	logic [23:1] IrcAddr,IrAddr;
	assign instr_start = enT1 & Nanod.Ir2Ird;
	assign instr_opcode = Ir;
	assign instr_address = IrAddr;
	...
	always_ff @( posedge Clks.clk) begin
		if(Clks.pwrUp) begin
			...
			// Mirror dataIo's delayed IRC capture qualifier so the address
			// follows the same bus word that is latched into Irc.
			if(enT1)
				addrXToIrc <= 1'b0;
			else if(enT3)
				addrXToIrc <= Nanod.toIrc;
			if(addrXToIrc & Clks.enPhi2)
				IrcAddr <= eab;
			if(enT1 & !Nanod.Ir2Ird & microLatch[0])
				IrAddr <= IrcAddr;
		end
		if( enT1) begin
			if( Nanod.Ir2Ird)
...
localparam NANO_FTU_CONST = 1;
	...
	always_ff @( posedge Clks.clk) begin
		if( enT4) begin
			...
			Nanod.dbin2Abd <= nanoLatch[NANO_DBIN2ABD];
			Nanod.dbin2Dbd <= nanoLatch[NANO_DBIN2DBD];
	...
	// The AU source select feeds mutually-exclusive bus muxes.  Register it at
	// T4, as the surrounding nanocode controls are, so a changing nanocode word
	// cannot momentarily enable the old FTU source and the new AU source at once.
	always_ff @(posedge Clks.clk) begin
		if (Clks.pwrUp)
			{Nanod.au2Db, Nanod.au2Ab, Nanod.au2Pc} <= '0;
		else if (enT4) begin
			Nanod.au2Db <= (nanoLatch[NANO_AUOUT + 1:NANO_AUOUT] == 2'b01);
			Nanod.au2Ab <= (nanoLatch[NANO_AUOUT + 1:NANO_AUOUT] == 2'b10);
			Nanod.au2Pc <= (nanoLatch[NANO_AUOUT + 1:NANO_AUOUT] == 2'b11);
		end
	end
...
module uRom( input clk, input [UADDR_WIDTH-1:0] microAddr, output logic [UROM_WIDTH-1:0] microOutput);
	...
	initial begin
		...
		// filename only works when the simulator happens to run from this source
		// directory and otherwise silently leaves the 68000 control store as X.
