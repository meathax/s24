...
module tb_gground_boot;
    ...
    // Same truncation trap for the four player joystick words: the visual
    // build's 32-bit host_joy0..3 ports are used at the .joy0(host_joy0)
    // etc. instance connections below regardless of build mode, so an
    // undeclared identifier here silently becomes an implicit 1-bit net
    // permanently tied to 0 -- meaning no coin, start or player input can
    // ever reach the core in a batch run. Scheduled by native frame number
    // (matching tools/run_game_matrix.py's GameplayProfile.coin_frame/
    // start_frame/action_frame fields) via the COIN_FRAME/START_FRAME/
    // ACTION_FRAME/INPUT_HOLD_FRAMES plusargs below.
    logic [31:0] host_joy0=32'd0,host_joy1=32'd0,
                 host_joy2=32'd0,host_joy3=32'd0;
`endif
...
`endif
    ...
    // Ungated native-frame counter for scheduled coin/start injection (see
    // the vsync block below): video_frames only starts counting after
    // CPU-B's release, which is exactly the event coin/start may need to
    // cause, so scheduling against it would be circular.
    integer raw_frame_count=0;
    integer coin_frame=0,start_frame=0,input_hold_frames=4;
...
`ifdef S24_VISUAL
    always_comb begin
        ...
        host_audio_event_valid=1'b0;
        host_audio_event_type='0;
        host_audio_event_channel='0;
        host_audio_event_address='0;
        host_audio_event_value='0;
        host_audio_event_left='0;
        host_audio_event_right='0;
...
`endif
    always @(posedge clk) begin
        ...
        vsync_d<=vsync;
        // Scheduled coin/start injection for the non-visual batch harness
        // (see host_joy0..3's declaration comment above for why this was
        // previously impossible). Native-frame-numbered to match
        // tools/run_game_matrix.py's GameplayProfile.coin_frame/start_frame
        // fields; held for input_hold_frames native frames each, matching
        // a real coin switch/start button press-and-release rather than a
        // level held indefinitely (which some titles' coin-counter logic
        // would otherwise mis-read as repeated credits).
        `ifndef S24_VISUAL
        if(vsync && !vsync_d) begin
            raw_frame_count<=raw_frame_count+1;
            host_joy0[11]<=(coin_frame!=0) &&
                (raw_frame_count>=coin_frame) &&
                (raw_frame_count<coin_frame+input_hold_frames);
            host_joy0[10]<=(start_frame!=0) &&
                (raw_frame_count>=start_frame) &&
                (raw_frame_count<start_frame+input_hold_frames);
        end
        `endif
...
`endif
    ...
    initial begin
        ...
        // Hot Rod's boot/attract flow requires the player to hold the
        // accelerator pedal to advance past a prompt. The non-visual batch
        // harness has no live input, so PADDLE0 lets a run override the
        // idle default for the whole run -- e.g. PADDLE0=ff to simulate the
        // pedal held down throughout. host_paddle0 is a module PORT (driven
        // externally by s24_visual_main.cpp) in the S24_VISUAL build, so
        // this override only applies to the non-visual/batch build.
        `ifndef S24_VISUAL
        void'($value$plusargs("PADDLE0=%h",host_paddle0));
        `endif
        void'($value$plusargs("COIN_FRAME=%d",coin_frame));
        void'($value$plusargs("START_FRAME=%d",start_frame));
        void'($value$plusargs("INPUT_HOLD_FRAMES=%d",input_hold_frames));
...
`else
        ...
        while(!reached_target && i<max_clocks) begin
            ...
            if(progress_clocks!=0 && i>=next_progress) begin
                ...
                // Diagnostic only (no new state): pin down exactly which
                // stage of CPU-B's fetch/decrypt pipeline is stuck when
                // Binsn stops advancing -- FD1094 FSM state, the shared-bus
                // arbiter's view of CPU-B's request, and the opcode-cache
                // hit/fill/snoop lines. Removed once the Gain Ground
                // black-screen root cause is found.
                $display("%s cpuB-diag fstate=%0d busy=%b done=%b b_as_n=%b b_dtack_n=%b bus_req=%b bus_cpu=%b bus_ack=%b b_mem_req=%b b_mem_ack=%b bc_hit=%b bc_fill=%b bc_snoop=%b bc_window=%b bc_q_valid=%b io_cnt1=%b sub_reset=%b sub_reset_count=%0d gslow=%b gslow_count=%0d phi1=%b phi2=%b b_phi1=%b b_phi2=%b b_pc=%h",
                    game_name,dut.fd1094.fstate,dut.fd_busy,dut.fd_done,
                    dut.b_as_n,dut.b_dtack_n,dut.bus_req,dut.bus_cpu,
                    dut.bus_ack,dut.b_mem_req,dut.b_mem_ack,dut.bc_hit,
                    dut.bc_fill,dut.bc_snoop,dut.bc_window,dut.bc_q_valid,
                    dut.io_cnt[1],dut.sub_reset,dut.sub_reset_count,
                    dut.gground_slow,dut.gground_slow_count,
                    dut.phi1,dut.phi2,dut.b_phi1,dut.b_phi2,
                    dut.b_instr_address);
        ...
        $display("%s irq timer: data=%h value=%h mode=%h allow_a=%h allow_b=%h pend_a=%b pend_b=%b vblank=%b sprite=%b frc_a=%b frc_b=%b",
            ...
            dut.irq.vblank_irq,dut.irq.sprite_irq,dut.irq.frc_a,dut.irq.frc_b);
        if(row_diag) begin
