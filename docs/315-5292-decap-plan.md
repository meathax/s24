...
| P1 | Pixel lanes | Character nibble order, CPU byte lanes, RAM mirroring, and 9-bit wrap are only lightly covered by the existing synthetic tile test. | Trace data pads through the input latches and pixel serializer; compare against walking-nibble board tests. |
