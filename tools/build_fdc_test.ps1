[CmdletBinding()]
param(
    [string]$ModelDirectory = 'C:/tmp/s24_obj_fdc'
)
...
$sourceCopies = @(
    @('rtl/fdc/s24_fdc.sv', 's24_fdc.sv'),
    @('verif/tb_fdc.sv', 'tb_fdc.sv')
)
...
$arguments = @(
    '--binary','--timing','--assert','--top-module','tb_fdc',
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& verilator-safe sim (Join-Path $ModelDirectory 'Vtb_fdc.exe')
exit $LASTEXITCODE
