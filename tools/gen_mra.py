...
MAME_VERSION = "0288"
RBF = "Arcade-SegaSystem24"
# yyyymmdd per the Wiki_MiSTer MRA-file convention. Passed in rather than
# computed at generation time: Date.now()-equivalents are avoided elsewhere
# in this project for reproducibility, and a fixed release date is what
# actually belongs in a shipped MRA regardless of when the script re-runs.
MRA_TIMESTAMP = "20260808"
...
MRA_CONTROL_PROFILES = {
    ...
    MRA_CONTROL_HOTROD: MraControls(
        ...
        ("Accelerator", "-", "-", "-", "-", "-", "Start", "Coin", "Service", "Test", "Pause"),
...
class Game:
    ...
    # Bit 0: opt this set into s24_tile's flicker-blend heuristic (see
    # s24_core.sv's blend_capable). Only Bonanza Bros' translucency effect
    # has been verified against it; every other set must leave this clear
    # so a legitimately-toggling disable bit renders as blank, matching
    # MAME, instead of stale content from whatever that layer is really
    # being used for that frame.
    video_profile: int = 0
    cpu_profile: int = 0
...
GAMES = (
    ...
    Game("sspirits", "Scramble Spirits (World, Floppy Based)", 1988,
         ...
         mra_controls=MRA_CONTROL_SSPIRITS),
    ...
    Game("bnzabros", "Bonanza Bros (US, Floppy DS3-5000-07d? Based)", 1990,
         ...
         mra_controls=MRA_CONTROL_BNZABROS, profile_version=1, video_profile=1),
)
...
def generate(game: Game, out_dir: pathlib.Path) -> pathlib.Path:
    ...
    lines.extend((
        ...
        # including the reference tool's own sample file. The current
        # official convention omits it entirely rather than propagate it.
    ...
    lines.extend((
        ...
        "  </rom>",
        f"  <mratimestamp>{MRA_TIMESTAMP}</mratimestamp>",
        "</misterromdescription>",
    ))
