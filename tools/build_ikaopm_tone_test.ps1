[CmdletBinding()]
param([string]$ModelDirectory = 'C:/tmp/s24_obj_ikaopm_tone')
...
$sources = @(
    ...
    'rtl/audio/s24_opm.sv','verif/tb_ikaopm_tone.sv'
)
$arguments = @(
    '--binary','--timing','--assert','-O3','--output-split','20000',
    '--top-module','tb_ikaopm_tone','--Mdir',$ModelDirectory,'--Wno-fatal',
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& verilator-sim-safe (Join-Path $ModelDirectory 'Vtb_ikaopm_tone.exe')
exit $LASTEXITCODE
