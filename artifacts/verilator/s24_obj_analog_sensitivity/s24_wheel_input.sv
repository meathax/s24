...
module s24_wheel_input (
    ...
    assign speed_scaled = speed_base * sensitivity;
    assign rough_rate_scaled = rough_rate * sensitivity;
    ...
    always_comb begin
        ...
        if (speed_mag != 8'd0) begin
            if (stick_x[7]) stick_step = -$signed({1'b0,speed_mag});
            else            stick_step =  $signed({1'b0,speed_mag});
        end
